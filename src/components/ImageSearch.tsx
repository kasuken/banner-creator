...
const ImageSearch: React.FC<ImageSearchProps> = ({ setBackgroundImage }) => {
    ...
    return (
        <div className="w-full">
            <label htmlFor="image-search" className="block mb-2 text-sm font-medium text-gray-700">
                Background Image
            </label>
            ...
            {error && (
                <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm flex items-center gap-2" role="alert">
            ...
            {images.length > 0 && (
                <div className="mt-4 grid grid-cols-3 gap-3 max-h-80 overflow-y-auto p-1 rounded-lg" role="list" aria-label="Search results">
                    {images.map((image) => (
                        <button
                            key={image.id}
            ...
            {!loading && query && images.length === 0 && (
                <div className="mt-4 text-center py-8 text-gray-500" role="status">
